sap.ui.define([
	...
	return Controller.extend("sap.demo.bpmrulesshoppingcart.view.Product", {
		...
		_addProduct: function(oProductToBeAdded) {
			...
			if (rulepkgName === undefined || rulepkgName === "" || rulepkgName === null) {// Demo Scenario
				var sProductCatToBeAdded = oProductToBeAdded.Category;
				var oRuleModelEntry = oRulesModelEntries[sProductCatToBeAdded];
				if (oRuleModelEntry === undefined) {
					...
					oRulesModelEntries[sProductCatToBeAdded] = oModel;

				} else {
					// update existing entry
					oRuleModelEntry[0].Quantity += 1;
				}

			} else {// Workshop Scenario
				var sProductCatToBeAddedForWorkshop = oProductToBeAdded.Category;
				var sProductSoldbyToBeAddedForWorkshop = oProductToBeAdded.SupplierName;
				var arrayIndexingForWorkshop = sProductCatToBeAddedForWorkshop + '-' + sProductSoldbyToBeAddedForWorkshop;
				var oRuleModelEntryForWorkshop = oRulesModelEntries[arrayIndexingForWorkshop];
				if (oRuleModelEntryForWorkshop === undefined) {
					// create new entry
				
					oRuleModelEntryForWorkshop = {};
					oRuleModelEntryForWorkshop.__type__ = "Product";
					oRuleModelEntryForWorkshop.Category = oProductToBeAdded.Category;
					oRuleModelEntryForWorkshop.Quantity = 1;
					oRuleModelEntryForWorkshop.Soldby = oProductToBeAdded.SupplierName;
					oModel.push(oRuleModelEntryForWorkshop);
					...
					oRulesModelEntries[arrayIndexingForWorkshop] = oModel;
				} else {
					// update existing entry
					oRuleModelEntryForWorkshop[0].Quantity += 1;
				}
			}
